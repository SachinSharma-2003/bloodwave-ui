...
CREATE TABLE public.donors (
  ...
  is_available BOOLEAN GENERATED ALWAYS AS (
    last_donated IS NULL OR last_donated <= CURRENT_DATE - INTERVAL '90 days'
  ) STORED,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
-- Create function to automatically create donor profile when user signs up as donor
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  -- This will be called from the application when user completes registration
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;